...
export default function Malla(props) {
	...
	const onChangeComplete = (event, value, reason) => {
		if (reason === 'select-option') {
			setInput('');
			if (typeof materiasSelect.find((e) => e.key === value.codigo) === 'undefined') {
	...
	return (
		<div className={classes.root}>
			...
			<><Grid container
				...
				<Grid >
					<Typography variant='subtitle2' color='textSecondary'>Seleccionadas</Typography>
				</Grid>

			</Grid>

			<Grid container={true} spacing={3} justify='center' alignItems='center'>
				{materiasSelect.map(mat => generarCelda(mat, materiasSelect.length))}
			</Grid>
			</>:<></>}
			...
			{materiasMalla && materiasMalla.length > 0 ?
				<><Grid container
					...
					alignItems="flex-start" >
					<Grid >
						<Typography variant='subtitle2' color='textSecondary'>Recomendadas</Typography>
					</Grid>

				</Grid> 
				
				<Grid container={true} spacing={3} justify='center' alignItems='center'>
				{materiasMalla.map(mat => generarCelda(mat, materiasMalla.length))}
				</Grid>
				</> : <></>
			}
