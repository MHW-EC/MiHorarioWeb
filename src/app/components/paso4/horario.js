  ...
  ({ classes, data, formatDate, ...restProps }) => (
    ...
    >
      <div className={classes.container}>
        <div className={classes.title}>{data.codigo}</div>
        <div className={classes.title}>{data.nombre}</div>
        <div className={classes.text}>PAR {data.paralelo}</div>
...
function Demo(props) {
  ...
  useEffect(() => {
    if (props.appointments) {
      let sorted = [...props.appointments].sort(function (a, b) {
  ...
  return appointments &&
    ...
    <Paper>
      <Scheduler data={appointments} locale="es-ES" firstDayOfWeek={firstDay}>
        ...
        <Resources palette={[Colors.red]} data={recursos} />
      </Scheduler>
    </Paper>
  ) : null;
}
